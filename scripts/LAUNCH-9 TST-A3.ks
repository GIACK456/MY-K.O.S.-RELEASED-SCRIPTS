...
SET velocyty_semi TO SQRT( GM / ((periapsis + apoapsis + 400000) / 2) ).

SET velocyty_apo TO angular_momentum / ((apoapsis + periapsis + 400000) / 2).
...
SET velocyty_apo TO (SQRT((GM) / (wapoapiss))).

SET angular_momentum TO (SQRT(2 * GM)) / (SQRT(((apoapsis + 200000) * (periapsis + 200000)) / (periapsis + apoapsis + 200000))).

SET delta_v TO (isp * 9.81 * (ln(MASS / DRYMASS))).

SET velocyty_semi TO angular_momentum / wapoapiss.
...
WAIT UNTIL burn_duration_halved > ORBIT:ETA:apoapsis - 20.
