...
UNTIL AG6 = AG2 OR AG5 = AG2 OR AG10 = AG2 OR (aly < 9 AND check = 1) {
     ...
     IF aly > 70000 AND check6 = 1 {
          SET phrase_old TO (" ON NOUR OWN ").
          SET phrase_prt TO (" WE ARE IN SPACE ").
          SET phrase_net TO (" RCS LATERAL FIRE START ").
          SET check7 TO 1.
     }

    IF aly > 80020 AND check7 = 1 {
          SET phrase_old TO (" WE ARE IN SPACE ").
          SET phrase_prt TO (" RCS LATERAL FIRE ON ").
          SET phrase_net TO (" APPROACHING APOGEE ").
          SET check8 TO 1.
     }


     IF (old_aly_check > aly_check) AND check8 = 1 {
          SET phrase_old TO (" RCS FIRE ON ").
          SET phrase_prt TO (" APPROACHING APOGEE ").
          SET phrase_net TO ("RCS FIRE OFF").
          SET check TO 1.
          SET check9 TO 1.
     }


    IF aly < 80020 AND check9 = 1 {
        SET phrase_old TO (" APPROCHING APOGEE ").
        SET phrase_prt TO (" RCS LATERAL FIRE OFF ").
        SET phrase_net TO (" WE CARE REENTERING THE ATMOSPHERE         !").
        SET checK10 TO 1.
    }


     IF aly < 70000 AND check > 0 AND checK10 = 1 {
          SET phrase_old TO (" APPROACHING APOGEE ").
          SET phrase_prt TO ("  WE CARE REENTERING THE ATMOSPHERE         !").
          SET phrase_net TO (" RCS ON AND RETROGRADE ").
          SET check11 TO 1.
     }


     IF aly < 65000 AND check > 0 AND check11 = 1 {
          SET phrase_old TO ("  WE CARE REENTERING THE ATMOSPHERE         ! ").
          SET phrase_prt TO (" RCS ON AND RETROGRADE ").
          SET phrase_net TO (" OPENING PARACHUTES "). 
          SET check12 TO 1.
     }



     IF aly < 36000 AND check > 0 AND check12 = 1 {
          SET check13 TO 1.
     } 
     ...
     IF (AG5 = AG2 OR AG10 = AG2) AND velocità1 < 50 AND check13 > 0 {
    ...
    IF (AG5 = AG2 OR AG10 = AG2) AND aly < 9 AND check14 > 0 {
    ...
    IF aly < 11010 AND check > 0 {
         SET phrase_old TO (" RCS ON AND RETROGRADE ").
         SET phrase_prt TO (" OPENING PARACHUTES ").
         SET phrase_net TO (" LANDING FIRE ").
         SET check12 TO 1.
     }

    IF velocità1 < 12 AND check > 0 {
        SET phrase_old TO (" OPENING PARACHUTES ").
         SET phrase_prt TO (" LANDING FIRE ").
         SET phrase_net TO (" WE ARE LANDED ").
         SET check13 TO 1.
     }

         IF aly < 9 AND check13 > 0 {
        SET phrase_old TO (" LANDING FIRE ").
